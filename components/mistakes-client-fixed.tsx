...
export function MistakesClientFixed() {
    ...
    return (
      <div className="glass-card rounded-[32px] p-8 text-center text-white">
        ...
        <p className="mt-3 text-white/80">
          틀린 문제가 생기면 자동으로 이곳에 쌓입니다.
        </p>
        <Link
          href="/quiz?mode=all"
          className="mt-6 inline-flex rounded-full bg-slate-900 px-5 py-3 text-sm font-bold text-white"
        >
          전체 랜덤 퀴즈 시작
        </Link>
      </div>
      ...
      {items.map((item) => (
        <article key={`${item.questionId}-${version}`} className="glass-card rounded-[28px] p-5 text-white">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-white/85">
            <span className="rounded-full bg-white/10 px-3 py-2 text-white">
              오답 횟수 {item.wrongCount}
            </span>
            <span className="rounded-full bg-white/10 px-3 py-2 text-white">
              재도전 횟수 {item.retryCount}
            </span>
            <button
              type="button"
              onClick={() => {
